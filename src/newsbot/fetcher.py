...
def _is_same_event(article: Article, other: Article) -> bool:
    ...
    if shared_topics and shared_actions and (shared_regions or shared_actors):
        return True

    if shared_topics and shared_regions and shared_actors and text_similarity >= 0.25:
        return True

    if shared_topics and shared_actions and text_similarity >= 0.32:
        return True
...
def _fingerprint_topics(lower_text: str) -> set[str]:
    topics: set[str] = set()
    topic_groups = {
        "tokenization_rwa": ["RWA", "real world asset", "代幣化", "tokenization", "tokenized securities"],
        "stablecoin": ["stablecoin", "穩定幣"],
        "virtual_asset": ["virtual asset", "虛擬資產", "digital asset"],
...
def _fingerprint_actions(lower_text: str) -> set[str]:
    ...
    action_groups = {
        ...
        "launch_adoption": PRODUCT_INSTITUTION_ADOPTION_KEYWORDS + ["推出", "上線", "launch", "launches"],
...
def _fingerprint_regions(lower_text: str, source: str) -> set[str]:
    ...
    region_terms = {
        ...
        "us": ["美國", "us", "u.s.", "united states", "SEC", "FINRA"],
        "japan": ["日本", "japan"],
    ...
    for region, keywords in region_terms.items():
        if any(_contains_keyword(lower_text, keyword) or keyword.lower() in lower_source for keyword in keywords):
            regions.add(region)
    return regions
