...
def _product_event_level(title: str, summary: str, source: str) -> int | None:
    text = f"{title}\n{summary}".lower()
    single_product_hits = _count_contains(text, SINGLE_PRODUCT_SIGNAL_KEYWORDS) + _product_identifier_hits(text)
    ...
    if (
        single_product_hits >= 1
        and routine_action_hits >= 1
        and product_field_hits >= 2
