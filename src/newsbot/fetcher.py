...
def _is_relevant(title: str, summary: str, source: str, published_at: datetime | None) -> bool:
    ...
    if _is_non_focus_entity(lower_text):
        return False

    if _is_investor_or_product_content(lower_text):
        return False

    if _is_non_focus_tech_vendor_article(lower_text, source):
        return False

    if not _has_required_focus(lower_text, source):
        return False
...
def _is_non_focus_entity(lower_text: str) -> bool:
    return any(_contains_keyword(lower_text, keyword) for keyword in NON_FOCUS_ENTITY_KEYWORDS)
...
def _is_non_focus_tech_vendor_article(lower_text: str, source: str) -> bool:
    tech_hit = any(_contains_keyword(lower_text, keyword) for keyword in TECH_VENDOR_SIGNAL_KEYWORDS)
    if not tech_hit:
        return False

    return not _has_required_focus(lower_text, source)


def _has_required_focus(lower_text: str, source: str) -> bool:
    if any(_contains_keyword(lower_text, keyword) for keyword in BROKERAGE_NAMES):
        return True

    if any(_contains_keyword(lower_text, keyword) for keyword in BROKERAGE_CORE_TERMS):
        return True

    if any(_contains_keyword(lower_text, keyword) for keyword in MARKET_RULE_CORE_TERMS):
        return True

    lower_source = source.lower()
    official_source = any(keyword.lower() in lower_source for keyword in OFFICIAL_SOURCES)
    if official_source and any(
