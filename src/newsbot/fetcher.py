...
def fetch_articles(lookback_hours: int, sources: list[NewsSource] | None = None) -> list[Article]:
    ...
    for source in sources or RSS_SOURCES:
        ...
        for entry in parsed.entries:
            ...
            event_key = _event_key(title, clean_summary, source.name)
            ...
            if event_key and event_key in seen_event_keys:
                for index, (existing_score, existing_article) in enumerate(scored_articles):
                    if _event_key(existing_article.title, existing_article.summary, existing_article.source) == event_key:
...
def _event_key(title: str, summary: str = "", source: str = "") -> str | None:
    lower_title = title.lower()
    lower_text = f"{title}\n{summary}".lower()

    if _has_emerging_product_signal(lower_text):
        if any(
            ...
            return f"emerging_product_multi_institution:{_emerging_product_family(lower_text)}:{_region_key(lower_text, source)}"

        if _count_contains(lower_text, PRODUCT_LEGAL_OPENING_KEYWORDS):
            return f"emerging_product_regulation:{_emerging_product_family(lower_text)}:{_region_key(lower_text, source)}"

        if any(_contains_keyword(lower_text, keyword) for keyword in ["交易所", "清算機構", "clearing", "settlement"]):
            return f"emerging_product_infrastructure:{_emerging_product_family(lower_text)}:{_region_key(lower_text, source)}"

        if _count_contains(lower_text, PRODUCT_BUSINESS_MODEL_KEYWORDS) or _count_contains(
            lower_text, PRODUCT_MECHANICS_CHANGE_KEYWORDS
        ):
            institution = _primary_institution_key(lower_text)
            return f"emerging_product_business_model:{_emerging_product_family(lower_text)}:{institution}"
...
def _emerging_product_family(lower_text: str) -> str:
    if any(_contains_keyword(lower_text, keyword) for keyword in ["RWA", "real world asset", "代幣化"]):
        return "tokenization_rwa"
    if any(_contains_keyword(lower_text, keyword) for keyword in ["stablecoin", "穩定幣"]):
        return "stablecoin"
    if any(_contains_keyword(lower_text, keyword) for keyword in ["virtual asset", "虛擬資產", "digital asset"]):
        return "virtual_asset"
    return "emerging_product"


def _region_key(lower_text: str, source: str) -> str:
    region_terms = [
        ("英國", "uk"),
        ("uk", "uk"),
        ("japan", "japan"),
    ...
    for keyword, key in region_terms:
        if _contains_keyword(lower_text, keyword) or keyword in source.lower():
            return key
    return "global"


def _primary_institution_key(lower_text: str) -> str:
    for keyword in BROKERAGE_NAMES:
        if _contains_keyword(lower_text, keyword):
            return _dedupe_key(keyword)
    for keyword in ["貝萊德", "摩根大通", "BlackRock", "JPMorgan", "JP Morgan", "交易所", "清算機構", "銀行"]:
        if _contains_keyword(lower_text, keyword):
            return _dedupe_key(keyword)
    return "multi_or_unspecified"
...
def _product_event_level(title: str, summary: str, source: str) -> int | None:
    ...
    multi_institution_or_model = _count_contains(text, PRODUCT_BUSINESS_MODEL_KEYWORDS) or (
